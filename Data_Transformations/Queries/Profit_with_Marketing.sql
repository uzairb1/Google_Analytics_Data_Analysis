select sum(t1.Subscribed)*20 as Revenue,(sum(t1.Subscribed)*20)-(select sum(t3.monthly_total_spend) from `my-project-1535534415901.formel_Uzi.formel_marketing` t3) as Profit, 
(select sum(t3.monthly_total_spend) from `my-project-1535534415901.formel_Uzi.formel_marketing` t3) as Marketing_Costs,
